...
function expectedClock(at, opts = {}) {
  // Fork note: this fork renders an unpadded hour ("9:33 PM") unless an explicit
  // hourCycle or seconds is requested, matching the usage line. Upstream always
  // pads. See formatClockTime in src/render/format-reset-time.ts.
  const explicitClock = Boolean(opts.hourCycle || opts.showSeconds);
  const timeOpts = { hour: explicitClock ? '2-digit' : 'numeric', minute: '2-digit' };
...
test('renderPromptCacheLine holds the same value as the render goes stale', () => {
  ...
  assert.equal(first, `Cache ⏱ ${expectedClock(anchor + 3_600_000)}`);
...
test('renderPromptCacheLine prefers the detected TTL over the 5m default', () => {
  ...
  assert.equal(
    stripAnsi(renderPromptCacheLine(ctx, now) ?? ''),
    `Cache ⏱ ${expectedClock(now - 10 * 60_000 + 3_600_000)}`,
  );
});
...
test('renderPromptCacheLine ignores a TTL that is not a real tier', () => {
  ...
  assert.equal(
    stripAnsi(renderPromptCacheLine(ctx, now) ?? ''),
    `Cache ⏱ ${expectedClock(now + 300_000)}`,
  );
});
...
test('renderPromptCacheLine follows hourCycle and showClockSeconds', () => {
  ...
  assert.equal(
    stripAnsi(renderPromptCacheLine(ctx, now) ?? ''),
    `Cache ⏱ ${expectedClock(now + 300_000, { hourCycle: 'h23', showSeconds: true })}`,
  );
});
