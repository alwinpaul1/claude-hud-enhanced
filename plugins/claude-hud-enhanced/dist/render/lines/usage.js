 ...
 */
export function formatStaleUsageMarker(staleSince, colors, now = Date.now()) {
...
function renderUsageBody(ctx, labelOptions) {
    ...
    const scopedPart = (w, withReset = true) => usageCompact
    ...
    const suffixOf = (windows) => windows.length ? ' | ' + windows.map((w) => scopedPart(w)).join(' | ') : '';
    ...
    const withScoped = (renderAnchor, anchorResetAt = ctx.usageData?.sevenDayResetAt ?? null) => {
