...
export function getIpcDir(homeDir = os.homedir()) {
    return path.join(getHudPluginDir(homeDir), 'daemon');
}
export function getIpcPath(homeDir = os.homedir()) {
    ...
    if (!onDrvFs && Buffer.byteLength(preferred, 'utf8') <= 100)
        return preferred;
    const key = createHash('sha256')
        .update(path.resolve(getClaudeConfigDir(homeDir)))
        .digest('hex')
...
export function getSpawnLockPath(homeDir = os.homedir()) {
    return path.join(getIpcDir(homeDir), 'hud.spawn.lock');
}
export function getPidPath(homeDir = os.homedir()) {
