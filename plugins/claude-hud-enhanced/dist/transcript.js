...
function fileEndsWithNewline(transcriptPath, size) {
    if (size === 0)
        return true;
    let fd;
    try {
 ...
 */
function readTranscriptCacheRaw(transcriptPath) {
    try {
...
function writeTranscriptCache(transcriptPath, state, data, resume) {
    try {
        const cachePath = getTranscriptCachePath(transcriptPath, os.homedir());
        ...
        writeJsonCacheAtomic(cachePath, payload, (err) => debug('Failed to write transcript cache:', err instanceof Error ? err.message : err));
...
export async function parseTranscript(transcriptPath) {
    ...
    if (cacheRaw?.acc != null
        ...
        try {
            ...
            for (const t of a.tools)
                toolMap.set(t.id, reviveToolEntry(t));
            for (const ag of a.agents)
                agentMap.set(ag.id, reviveAgentEntry(ag));
            for (const s of a.skills)
                skillSet.add(s);
            for (const m of a.mcpServers)
                mcpServerSet.add(m);
            latestTodos = a.todos.map((td) => ({ ...td }));
            for (const [k, v] of a.taskIdToIndex)
                taskIdToIndex.set(k, v);
            for (const [k, iso] of a.queueCompletion)
                queueCompletionMap.set(k, new Date(iso));
            for (const [id, u] of a.messageUsage)
                usageByMessageId.set(id, { ...u });
    ...
    try {
        ...
        const fileStream = createReadStreamImpl(canonicalTranscriptPath, transcriptState.size > 0 ? { end: transcriptState.size - 1 } : undefined);
...
function extractTarget(toolName, input) {
    ...
    switch (toolName) {
        ...
        case 'Write':
        case 'Edit':
            return sanitizeTarget(input.file_path ?? input.path);
        case 'Glob':
            return sanitizeTarget(input.pattern);
        case 'Grep':
            return sanitizeTarget(input.pattern);
        case 'Skill':
