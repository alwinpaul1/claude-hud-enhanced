...
const MODEL_PRICING: Array<{ pattern: RegExp; pricing: ModelPricing }> = [
  // MiniMax M3 charges no cache-write premium, so cacheWriteUsdPerMillion is
  // null rather than a multiple of input. Row restored during the port:
  // upstream/main carries the covering test but lost the row itself.
