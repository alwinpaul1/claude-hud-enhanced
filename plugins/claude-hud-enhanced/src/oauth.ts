...
function readFromLibsecret(): string | null {
  if (process.platform !== 'linux') return null;
  try {
    const out = execFileSync(
      'secret-tool',
      ['lookup', 'service', LEGACY_KEYCHAIN_SERVICE_NAME, 'account', os.userInfo().username],
...
function readFromWindowsCredentialManager(): string | null {
  ...
  try {
    ...
    const script =
      `$c = Get-StoredCredential -Target '${LEGACY_KEYCHAIN_SERVICE_NAME}' -ErrorAction SilentlyContinue; ` +
...
export function readOAuthInfo(): OAuthInfo {
  ...
  if (!raw) {
    const empty: OAuthInfo = { subscriptionType: null, rateLimitTier: null };
    writeCache(empty);
    return empty;
  }
...
export function formatPlanLabel(info: OAuthInfo): string | null {
