...
async function withDeterministicSpeedCache(fn) {
  const tempConfigDir = await mkdtemp(
    path.join(tmpdir(), "claude-hud-render-"),
  );
  ...
  await writeFile(
    cachePath,
    JSON.stringify({ outputTokens: 1000, timestamp: 1000 }),
    "utf8",
  );
  Date.now = () => 2000;
...
test("renderSessionLine includes duration and formats large tokens", () => {
  ...
  assert.ok(
    line.includes("685k") || line.includes("685.0k"),
    "expected large input token display",
  );
...
test(
  "renderSessionLine displays project name from Windows cwd",
...
test("renderSessionLine supports combined context display", () => {
  ...
  const line = renderSessionLine(ctx);
  assert.ok(
    line.includes("7% (12k/200k)"),
    "should include percentage and token counts",
  );
});
...
test("render expanded layout supports remaining-based context display", () => {
  ...
  // 12345/200k = 6.17% raw, scale ≈ 0.026, buffer ≈ 858 → 7% buffered → 93% remaining
  assert.ok(
    logs.some((line) => line.includes("Context") && line.includes("93%")),
    "expected remaining percentage on context line",
  );
});
...
test("render expanded layout supports combined context display", () => {
  ...
  assert.ok(
    logs.some(
      (line) => line.includes("Context") && line.includes("7% (12k/200k)"),
    ),
...
test("label color overrides apply across shared secondary text surfaces", () => {
  ...
  assert.ok(
    renderEnvironmentLine(ctx)?.includes(
      `${expected}2 CLAUDE.md | 1 rules\x1b[0m`,
    ),
  );
  assert.ok(
    renderMemoryLine({
      ...
      config: {
        ...ctx.config,
        lineLayout: "expanded",
        display: { ...ctx.config.display, showMemoryUsage: true },
      },
    })?.includes(`${expected}Approx RAM\x1b[0m`),
  );
...
test("renderProjectLine omits duration when showDuration is false", () => {
  ...
  const line = renderProjectLine(ctx);
  assert.ok(
    !line?.includes("12m 34s"),
    "should not include session duration when disabled",
  );
});
...
test("renderProjectLine includes speed when showSpeed is true and speed is available", async () => {
  await withDeterministicSpeedCache(async () => {
    ...
    const line = renderProjectLine(ctx);
    assert.ok(
      line?.includes("out: 1000.0 tok/s"),
      "should include deterministic speed",
    );
  });
});
...
test("render expanded layout includes speed and duration on the project line", async () => {
  await withDeterministicSpeedCache(async () => {
    ...
    assert.ok(
      projectLine.includes("out: 1000.0 tok/s"),
      "should include deterministic speed",
    );
    assert.ok(
      projectLine.includes("⏱️  12m 34s"),
...
test("renderSessionLine omits project name when showProject is false", () => {
  ...
  const line = renderSessionLine(ctx);
  assert.ok(
    !line.includes("my-project"),
    "should not include project name when showProject is false",
  );
  assert.ok(
    line.includes("git:("),
    "should still include git status when showProject is false",
  );
});
  ...
  ctx.transcript.agents = [
    {
      ...
      model: "haiku",
      description:
        "A very long description that should be truncated in the HUD output",
      status: "completed",
      startTime: new Date(0),
...
test("renderTodosLine truncates long todo content", () => {
  ...
  ctx.transcript.todos = [
    {
      content:
        "This is a very long todo content that should be truncated for display",
      status: "in_progress",
    },
  ];
...
test("renderSessionLine does not add a synthetic subscriber label from usageData", () => {
  ...
  assert.ok(
    !line.includes("Max"),
    "should not include plan name derived outside stdin",
  );
});
...
test("renderSessionLine shows API label when API key auth is active", () => {
  ...
  try {
    ...
    assert.ok(
      line.includes("API"),
      "should include API label for API key auth",
    );
    assert.ok(
      !line.includes("Max"),
      "should not include subscriber plan label",
    );
  } finally {
...
test("renderSessionLine shows 7d reset countdown in text-only mode", () => {
  ...
  assert.ok(
    line.includes("7d: 85%"),
    `should include 7d label and percentage: ${line}`,
  );
  assert.ok(
    line.includes("(1d 4h)"),
...
test("renderSessionLine shows weekly-only usage without a ghost 5h section", () => {
  ...
  assert.ok(
    !line.includes("5h:"),
    `should not render a ghost 5h section: ${line}`,
  );
  assert.ok(
    line.includes("7d:"),
    `should render the weekly window when it is the only usage value: ${line}`,
  );
  assert.ok(
    line.includes("13%"),
    `should render the weekly percentage: ${line}`,
  );
});
...
test("renderUsageLine shows reset countdown in days when >= 24 hours", () => {
  ...
  assert.ok(
    !plain.includes("151h"),
    `should avoid raw hour format for long durations: ${plain}`,
  );
});
...
test("renderUsageLine shows 7d reset countdown in text-only mode", () => {
  ...
  assert.ok(
    line.includes("5h: 45%"),
    `should include 5h text-only usage: ${line}`,
  );
  assert.ok(
    line.includes("7d: 85%"),
    `should include 7d text-only usage: ${line}`,
  );
  assert.ok(
...
test("renderUsageLine shows 7d reset countdown in bar mode when above threshold", () => {
  ...
  assert.ok(
    line.includes("45%"),
    `should include 5h percentage in bar mode: ${line}`,
  );
  ...
  assert.ok(
    line.includes("|"),
    `should render both usage windows above the threshold: ${line}`,
  );
});
...
test("renderUsageLine shows weekly-only usage without a ghost 5h section", () => {
  ...
  assert.ok(
    !line.includes("|"),
    `should not render a separator for a missing 5h window: ${line}`,
  );
});
...
test("renderUsageLine shows limit reset in days when >= 24 hours", () => {
  ...
  assert.ok(
    /resets \d+d( \d+h)?/.test(plain),
    `expected day/hour reset format, got: ${plain}`,
  );
  assert.ok(
    !plain.includes("151h"),
    `should avoid raw hour format for long durations: ${plain}`,
...
test("renderSessionLine uses custom critical colors for limit-reached usage state", () => {
  ...
  assert.ok(
    criticalLine.includes("\x1b[35m⚠ Limit reached"),
    `expected custom critical color, got: ${JSON.stringify(criticalLine)}`,
  );
});
...
test("renderUsageLine uses custom usage palette overrides", () => {
  ...
  assert.ok(
    line.includes("\x1b[36m25%\x1b[0m"),
...
test("renderSessionLine hides usage when showUsage config is false (hybrid toggle)", () => {
  ...
  assert.ok(
    !line.includes("5h:"),
    "should not show usage when showUsage is false",
  );
  assert.ok(
    !line.includes("Pro"),
    "should not show plan name when showUsage is false",
  );
});
...
test("renderSessionLine uses buffered percent when autocompactBuffer is enabled", () => {
  ...
  assert.ok(
    line.includes("39%"),
    `expected buffered percent 39%, got: ${line}`,
  );
});
...
test("render preserves regular spaces instead of non-breaking spaces", () => {
  ...
  assert.ok(
    ...
    "output should not include non-breaking spaces",
  );
});
...
test("renderSessionLine handles missing showFileStats config (backward compatibility)", () => {
  ...
  assert.ok(
    !line.includes("!2"),
    "should not show file stats when config missing",
  );
});
...
test("render expanded layout honors custom elementOrder including activity placement", () => {
  ...
  const combinedIndex = lines.findIndex(
    (line) => line.includes("Usage") && line.includes("Context"),
  );
  ...
  assert.ok(
    projectIndex < combinedIndex,
    "combined usage/context line should follow project",
  );
  assert.ok(
    combinedIndex < memoryIndex,
    "memory line should follow combined usage/context",
  );
  assert.ok(
    memoryIndex < environmentIndex,
    "environment line should follow memory",
  );
  assert.ok(
    environmentIndex < agentIndex,
    "agent line should follow environment",
  );
...
test("render expanded layout omits elements not present in elementOrder", () => {
  ...
  assert.ok(
    output.includes("my-project"),
    "project should render when included",
  );
  assert.ok(output.includes("Read"), "tools should render when included");
  assert.ok(
    !output.includes("Context"),
    "context should be omitted when excluded",
  );
  assert.ok(!output.includes("Usage"), "usage should be omitted when excluded");
  assert.ok(
    !output.includes("Approx RAM"),
    "memory should be omitted when excluded",
  );
  assert.ok(
    !output.includes("CLAUDE.md"),
    "environment should be omitted when excluded",
  );
  assert.ok(
    !output.includes("planner"),
    "agents should be omitted when excluded",
  );
  assert.ok(
    !output.includes("todo-marker"),
    "todos should be omitted when excluded",
  );
});
...
test("render expanded layout combines usage and context when adjacent in elementOrder", () => {
  ...
  assert.equal(
    lines.length,
    1,
    "adjacent usage and context should share one expanded line",
  );
  assert.ok(lines[0].includes("Usage"), "combined line should include usage");
  assert.ok(
    lines[0].includes("Context"),
    "combined line should include context",
  );
  assert.ok(
    lines[0].includes("│"),
    "combined line should preserve the shared separator",
  );
});
...
test("render expanded layout keeps usage and context separate when not adjacent", () => {
  ...
  assert.equal(
    combinedLine,
    undefined,
    "usage and context should not combine when separated by another element",
  );
});
...
test("render compact layout keeps activity lines even when elementOrder omits them", () => {
  ...
  assert.ok(
    output.includes("todo-marker"),
    "compact mode should keep todos visible",
  );
});
